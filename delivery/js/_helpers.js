...
// если страница открыта на мобильном устройстве, то добавляем body класс touch
if (isMobile.any()) { // any - значит любой
	body.classList.add('touch');
	// получим стрелочки, чтобы с ними работать далее
	let arrow = document.querySelectorAll('.arrow');

	for (i = 0; i < arrow.length; i++) {
		// присвою переменной thisLink, объект, предсшествующий стрелке (ссылка)
		let thisLink = arrow[i].previousElementSibling;
		// присваиваю в переменную subMenu, объект, который находится сразу после стрелки (выпадающее меню)
		let subMenu = arrow[i].nextElementSibling;
		// присвою переменной thisArrow, текущую стрелку (текущая стрелка)
		let thisArrow = arrow[i];

		// повешу класс на ссылку
		thisLink.classList.add('parent');

		// повешу событие клик на стрелочку
		arrow[i].addEventListener('click', function () {
			// при клике на стрелку, у выпадающего меню - добавляется/убирается класс .open
			subMenu.classList.toggle('open');
			// при клике на стрелку, стрелке - добавить/убпать класс .active
			thisArrow.classList.toggle('active');
		});
	}
} else { // иначе, тоесть страница открыта на десктопе, то добавляем body класс mouse
	body.classList.add('mouse');
}

// ПРИМЕЧАНИЕ: Если убрать условие if/else, то на десктопе, выпадающее меню будет работать так же - по клику, как на тачскринах.
...
// ========================================================================
// Элементы форм checkbox и radio (с урока)

// CHECKBOX
// Обойду каждый div с классом .checkbox
$.each($('.checkbox'), function (indexInArray, valueOfElement) {
	// поищу в нем input и проверю, активен ли этот input (выделен ли он)
	if($(this).find('input').prop('checked') == true) { // и есл ида, то...
		// добавлю моему div с классом checkbox, класс .active
		$(this).addClass('active');
	}
});
// Повешу событие клика на .checkbox
$(document).on('click', '.checkbox', function(event) {
	// делаю проверку - если у нажатого div с классом .checkbox уже есть класс .active
	if($(this).hasClass('active')) { // если да, то...
		// убираю активность с input'а
		$(this).find('input').prop('checked', false);
	} else { // если нет, то ...
		// добавляю активность input'у
		$(this).find('input').prop('checked', true);
	}
	// и дополнительно добавляю/убираю класс .active для самого нажатого div склассом .checkbox
	$(this).toggleClass('active');
...
//RADIO
// Обойду каждый div с классом .radiobuttons__item
$.each($('.radiobuttons__item'), function(index, val) {
	// поищу в нем input и проверю, активен ли этот input (выделен ли он)
	if ($(this).find('input').prop('checked') == true) { // если да, то...
		// добавлю моему div с классом checkbox, класс .active
		$(this).addClass('active');
	}
});
// Повешу событие клика на .radiobuttons__item
$(document).on('click', '.radiobuttons__item', function(event) {
	// при нажатии, ищу родителя с классом .radiobuttons, в нем ищу все радиокнопки, убираю у них класс .active
	$(this).parents('.radiobuttons').find('.radiobuttons__item').removeClass('active');
	// при нажатии, ищу родителя с классом .radiobuttons, в нем ищу input, убираю у всех input выделенное значение
	$(this).parents('.radiobuttons').find('.radiobuttons__item input').prop('checked', false);
	// добавляю класс .active только нажатому элементу
	$(this).toggleClass('active');
	// в нем ищу input и добавляю ему checked
	$(this).find('input').prop('checked',true);
	return false;
});
