...
function forms() {
	...
	// FIELDS ================================================
	// .req - добавлять input'ам. Сделает input обязательным к заполнению.
	// .err - добавиться к input, который обязателен к заполнению, но не заполнен и была попытка отправки формы.
	...
	// .email - добавлять input'ам вместе с классом .req, если нужно сделать проверку на заполнение email.
	// data-type="search"
	// data-type="pass"
	...
	// QUANTITY
	$('.counter-btn').click(function (event) {
		var n = parseInt($(this).parent().find('.input-quantity').val());
		...
		$(this).parent().find('.input-quantity').val(n);
		return false;
	});
