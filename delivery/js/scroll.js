// Этот скрипт нужен для того, чтобы упралять содержимым секции, проделывать какие-то манипуляции со скроллом. Он содержит различные реакции на сролл.

// .sector - доваить класс блоку, до которого нужно прокручивать

// .view - класс добавляется сектору когда он только появляется.
// .active - класс добавляется сектору чуть позже, на 2/3 позже
// .scroll - к header автоматически добавляется класс .scroll. Так же он добавляется секторам
